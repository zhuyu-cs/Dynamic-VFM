...
def single_model_OODTrack(data_root, gpus):
    set_random_seed()
    ...
    for mode in ['2_4_8_16', '2_4_8','2_4_16','2_8_16','4_8_16','2_4','2_8','2_16','4_8','4_16','8_16','2', '4','8','16']:
        #! mean fusion model
        ...
        for tier in tier_list:
                    ...
                    if tiers[idx] == tier:
                        ...
                        with torch.no_grad():
                            if videos.shape[2]==300:
                                ...
                                stacked_input_behavior.append(behavior[:, :, -39:-9])  
                                stacked_input_behavior.append(behavior[:, :, -30:]) 
                            elif videos.shape[2]==239:
                                stacked_input_video = [videos[:, :, batch_index:batch_index+30, :, :] for batch_index in range(35, 239-24, 15)]  
                                stacked_input_video.append(videos[:, :, -39:-9, :, :])    
                                stacked_input_video.append(videos[:, :, -30:, :, :])                          
                                stacked_input_pupil_center = [pupil_center[:, :, batch_index:batch_index+30] for batch_index in range(35, 239-24, 15)]
                                stacked_input_pupil_center.append(pupil_center[:, :, -39:-9])  
                                stacked_input_pupil_center.append(pupil_center[:, :, -30:])  
                                stacked_input_behavior = [behavior[:, :, batch_index:batch_index+30] for batch_index in range(35, 239-24, 15)]
                                stacked_input_behavior.append(behavior[:, :, -39:-9])  
                                stacked_input_behavior.append(behavior[:, :, -30:]) 
                            ...
                            out = (model_mean(inputs=stacked_input_video.cuda(), 
                            ...
                            if videos.shape[2]==300 or videos.shape[2]==299:
                                ...
                                real_out = torch.zeros((189, out.shape[-1]))
                                for batch_index, index in zip(range(out.shape[0]), range(0, 189-9, 15)):
                                    real_out[index:index+15,...] = out[batch_index, ...]
                                real_out[-9:,...] = out[-1, -9:,...]
            ...
            if 'live' in tier:
                submission_filename = f"predictions_live_test_bonus_ood.parquet.brotli"
            else:
                submission_filename = f"predictions_final_test_bonus_ood.parquet.brotli"
            os.makedirs(path, exist_ok=True)
            os.makedirs(path+f'{mode}/', exist_ok=True)
            save_path = os.path.join(path+f'{mode}/', submission_filename) if path is not None else submission_filename
    ...
    for mode in ['0','2_4_8_16', '2_4_8','2_4_16','2_8_16','4_8_16','2_4','2_8','2_16','4_8','4_16','8_16','2', '4','8','16']:     
        ...
        model_adaptive.load_state_dict(weight, strict=True)
        for tier in tier_list:
            tier = f'{tier}_{track}'
            dataframes_pred = []
            for m in mice:
                ...
                for idx in range(len(tiers)):
                    if tiers[idx] == tier:
                        ...
                        with torch.no_grad():
                            ...
                            out = (model_adaptive(inputs=stacked_input_video.cuda(), 
                                    pupil_center=stacked_input_pupil_center.cuda(),
                                    behavior=stacked_input_behavior.cuda(),
                                    data_key=m, infer_mode=mode)
                                    .detach()
