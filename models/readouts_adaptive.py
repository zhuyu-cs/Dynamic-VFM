...
class AdaptiveFusion(nn.Module):
    ...
    def forward(self, x):
        B, _, T, N = x.shape
        mean = self.pool(x).squeeze(-1)
        attention = self.module(mean.permute(0,2,1)).permute(0,2,1).unsqueeze(-1)
        return (x * attention).sum(1).view(B, T, N)
...
class FullGaussian2d(Readout):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.fusion_block2 = AdaptiveFusion(in_channel=2)
        self.fusion_block3 = AdaptiveFusion(in_channel=3)
        self.fusion_block4 = AdaptiveFusion(in_channel=4)
        self.fusion_block5 = AdaptiveFusion(in_channel=5)
    ...
    def forward(self, x, sample=None, shift=None, out_idx=None, infer_mode='0', **kwargs):
        ...
        y2_up = F.interpolate(y2.unsqueeze(1).unsqueeze(1), size=(1,outdims), mode='nearest').squeeze(1)
        y4_up = F.interpolate(y4.unsqueeze(1).unsqueeze(1), size=(1,outdims), mode='nearest').squeeze(1)
        y8_up = F.interpolate(y8.unsqueeze(1).unsqueeze(1), size=(1,outdims), mode='nearest').squeeze(1)
        y16_up = F.interpolate(y16.unsqueeze(1).unsqueeze(1), size=(1,outdims), mode='nearest').squeeze(1)
        ...
        #! adaptive fusion
        y = y.view(-1, 15, outdims).unsqueeze(1)
        y2 = y2_up.view(-1, 15, outdims).unsqueeze(1)
        y4 = y4_up.view(-1, 15, outdims).unsqueeze(1)
        y8 = y8_up.view(-1, 15, outdims).unsqueeze(1)
        y16 = y16_up.view(-1, 15, outdims).unsqueeze(1)
        
        if infer_mode =='0':
            y = y.view(N, outdims)
        elif infer_mode =='2_4_8_16':
            y = self.fusion_block5(torch.cat([y, y2, y4, y8, y16], dim=1)).view(N, outdims)
        elif infer_mode =='2_4_8':
            y = self.fusion_block4(torch.cat([y, y2, y4, y8],dim=1)).view(N, outdims)
        elif infer_mode =='2_4_16':
            y = self.fusion_block4(torch.cat([y, y2, y4, y16],dim=1)).view(N, outdims)
        elif infer_mode =='2_8_16':
            y = self.fusion_block4(torch.cat([y, y2, y8, y16],dim=1)).view(N, outdims)
        elif infer_mode =='4_8_16':
            y = self.fusion_block4(torch.cat([y, y4, y8, y16],dim=1)).view(N, outdims)
        elif infer_mode =='2_4':
            y = self.fusion_block3(torch.cat([y, y2, y4],dim=1)).view(N, outdims)
        elif infer_mode =='2_8':
            y = self.fusion_block3(torch.cat([y, y2, y8],dim=1)).view(N, outdims)
        elif infer_mode =='2_16':
            y = self.fusion_block3(torch.cat([y, y2, y16],dim=1)).view(N, outdims)
        elif infer_mode =='4_8':
            y = self.fusion_block3(torch.cat([y, y4, y8],dim=1)).view(N, outdims)
        elif infer_mode =='4_16':
            y = self.fusion_block3(torch.cat([y, y4, y16],dim=1)).view(N, outdims)
        elif infer_mode =='8_16':
            y = self.fusion_block3(torch.cat([y, y8, y16],dim=1)).view(N, outdims) 
        elif infer_mode =='2':
            y = self.fusion_block2(torch.cat([y, y2],dim=1)).view(N, outdims)
        elif infer_mode =='4':
            y = self.fusion_block2(torch.cat([y, y4],dim=1)).view(N, outdims)
        elif infer_mode =='8':
            y = self.fusion_block2(torch.cat([y, y8],dim=1)).view(N, outdims)
        elif infer_mode =='16':        
            y = self.fusion_block2(torch.cat([y, y16],dim=1)).view(N, outdims)
